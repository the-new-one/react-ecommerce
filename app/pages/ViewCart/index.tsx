...
export const ViewCartItems = (props: any) => {
  ...
  let purchasedTotal: number = 0.0;

  const [actions, setActions] = useState<
    {key: string; id: number; value: number} | undefined
  >(undefined);
  ...
  const onDroppedItem = (item: Product) => {
    ...
    const newCartItem = userCartContext.cartItem.filter(
      (cart: Product) => cart.id !== item.id,
    );
  ...
  function onRenderCartItems({item}: ListRenderItemInfo<Product>) {
    return (
      <>
        ...
        <DV.DividerContainer margin="5px 0 0 0" />
      </>
    );
  }
  ...
  return (
    <CS.ContainerStyle style={style.parentHeight}>
                ...
                <View style={style.viewModalContainer}>
                  <View>
                    {userCartContext.cartItem.map((cart: Product) => {
                      purchasedTotal +=
                        cart.baseQuantity * parseFloat(cart.price);
                      return (
                        <View key={cart.id}>
                          <FS.FlexRowStyle>
                    ...
                    <Text style={[style.cartText, style.purchaseTotalPayment]}>
                      Total: $ {purchasedTotal}
                    </Text>
                  </View>
                  <View style={style.buttonPurchaseActions}>
                    <FS.FlexRowStyle>
                      <TouchableOpacity style={style.purchaseCloseModal} onPress={onProceedToBilling}>
                        <Text
                          style={[
                            style.cartText,
                            style.cartTextCenter,
                            style.cartTextWhite,
                          ]}>
                          Proceed to billing
                        </Text>
                      ...
                      <TouchableOpacity
                        ...
                        style={style.purchaseCloseModal}>
                        <Text
                          style={[
                            style.cartText,
                            style.cartTextCenter,
